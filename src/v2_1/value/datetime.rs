...
//!
//! [`VcardDateAndOrTime`] backs `BDAY`; [`VcardTimestamp`] backs `REV`. The
//! value is kept as its raw ISO 8601 text rather than decoded into calendar
//! fields, so it round-trips exactly; callers that need calendar semantics parse
//! the string themselves. Pure data, no escaping; the owning property's wire
//! name lives on [`crate::v2_1::prop::VcardProp::name`].
