...
//! model.
//!
//! `unescape` is the read codec (it resolves the RFC 2426 value escapes). On top
    ...
    pub fn decode(&self) -> VcardProp<'_> {
        ...
        let value = {
            ...
            match name.to_ascii_uppercase().as_str() {
                VCARD_FN | VCARD_NAME | VCARD_PROFILE | VCARD_LABEL | VCARD_TEL | VCARD_EMAIL
                | VCARD_MAILER | VCARD_TITLE | VCARD_ROLE | VCARD_AGENT | VCARD_NOTE
                | VCARD_PRODID | VCARD_SORT_STRING | VCARD_UID | VCARD_CLASS => {
                    VcardValue::Text(VcardText::decode(node))
                }
                VCARD_NICKNAME | VCARD_CATEGORIES => {
                    VcardValue::TextList(VcardTextList::decode(node))
                }
                VCARD_SOURCE | VCARD_URL => VcardValue::Uri(VcardUri::decode(node)),
                VCARD_PHOTO | VCARD_LOGO | VCARD_SOUND | VCARD_KEY => {
                    VcardValue::Binary(VcardBinary::decode(node))
                }
                VCARD_BDAY => VcardValue::DateAndOrTime(VcardDateAndOrTime::decode(node)),
                VCARD_REV => VcardValue::Timestamp(VcardTimestamp::decode(node)),
                VCARD_TZ => VcardValue::UtcOffset(VcardUtcOffset::decode(node)),
                ...
                VCARD_GEO => VcardValue::Geo(VcardGeo::decode(node)),
...
impl VcardBinary<'_> {
    /// Decode a binary value (inline base64 or external URI) from a syntax node.
    /// The form is told by the line's `VALUE` / `ENCODING` parameters; the value
    /// itself is kept here as its raw text.
    pub fn decode<'v>(node: &'v VcardValueNode<'_>) -> VcardBinary<'v> {
        VcardBinary(node.decode_scalar_at(0))
    }
}
...
impl VcardGeo<'_> {
    /// Decode the structured GEO value from a syntax node.
    pub fn decode<'v>(node: &'v VcardValueNode<'_>) -> VcardGeo<'v> {
        VcardGeo {
...
/// Resolve the RFC 2426 value escapes `\\` `\,` `\;` `\n`, borrowing when there
...
mod tests {
    ...
    #[test]
    fn decodes_the_structured_geo_value() {
        let node = VcardValueNode::parse("39.95;-75.1667");
        let geo = VcardGeo::decode(&node);
        assert_eq!(geo.latitude, Cow::Borrowed("39.95"));
        assert_eq!(geo.longitude, Cow::Borrowed("-75.1667"));
    }
}
