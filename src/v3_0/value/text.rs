...
//!
//! These back the bulk of RFC 2426 properties whose value is plain text: `FN`,
//! `TITLE`, `ROLE`, `NOTE`, `PRODID`, `TEL`, `EMAIL`, `MAILER`, `LABEL`,
//! `AGENT`, `UID`, `CLASS`, `NAME`, `PROFILE`, `SORT-STRING` for [`VcardText`],
//! and `NICKNAME` / `CATEGORIES` for [`VcardTextList`]. They are pure,
//! always-unescaped data; the escaping and the wire framing live entirely on the
//! syntax side ([`crate::v3_0::tree`]), so the same value type round-trips
//! through any property that shares the kind. The wire name that distinguishes
//! those properties is carried by [`crate::v3_0::prop::VcardProp::name`], not here.
