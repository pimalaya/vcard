//! # vCard 3.0 (RFC 2426)
//!
//! A vCard (RFC 2426) implementation built around **two representations of a
//! card, bridged by lenses**. The code is the living architecture; this is the
//! map.
//!
...
//!   byte. RFC 2426 line folding is normalised away: a folded line is unfolded
...
//! ([`tree::prop::n::N`], [`tree::param::language::LANGUAGE`]) is a zero-sized type
//! pinning a wire name to the decoded value it produces, plus the
//! `decode`/`encode` projections. Each property has its own hand-written module
//! under [`tree::prop`]; most share a value kind
...
//! [`ADR`](value::adr::VcardAdr), [`GEO`](value::geo::VcardGeo)) carry a cursor
//! that names their components. The wire names have a single source of truth:
//! the `VCARD_*` consts in [`prop`] and [`param`].
//!
//! ## Invariants
//!
...
//!   [`version`](crate::version), [`prop`] (property + names), [`param`] (parameter + names),
...
//!   [`leaf`](crate::tree::leaf) / [`line`](tree::line) / [`value`](tree::value) /
...
//!   value codec), and [`error`](crate::tree::error) (the parse errors).
//!
//! ## Status
//!
//! Every RFC 2426 property and parameter is modelled. Simple values share a
//! small set of value kinds; the structured ones (`N`, `ADR`, `ORG`, `GEO`) get
//! bespoke types, and the binary-bearing properties (`PHOTO`, `LOGO`, `SOUND`,
//! `KEY`) decode to a [`VcardBinary`](value::binary::VcardBinary) whose URI or
//! inline-base64 form is told by the `VALUE` / `ENCODING` parameters; anything
